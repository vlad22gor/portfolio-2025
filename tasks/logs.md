...
- 2026-08-05: Удалено остаточное затемнение в правом peach-padding рядом с hero-card GoomY cover. Пользовательский crop и численная residual-карта подтвердили вертикальное эхо прежней generated-границы: до исправления residual mean справа был `12.981` против `5.903` слева. Создана source-space маска точного flat background для обоих gutters (`x=0…185` и `x=1020…1205`, `y=450…1830`), exact core очищается полностью, а расширенная область мягко feather-ится у карточки; UI/текст/фото/рамка не перегенерировались. Проверки: residual mean справа `12.981 → 0.295`, слева `5.903 → 0.220`; 2× before/after, полный master и WebP-crop визуально проверены, вне screen mask `max delta = 0`. Обновлены canonical 1×/2× master и runtime `cover.webp` (`326962` bytes, PSNR `42.5182 dB`, SHA-256 `e8cb98956831c48df2a72689f91d4f1233187f9ef089d653841226c45b448188`). Предыдущая версия сохранена как `cover-clean-before-card-gutters-fix.webp` и соседние PNG с суффиксом `before-card-gutters-fix`.

- 2026-08-05: Удалена заметная ступенчатая полоса под hero-card в финальном GoomY cover. Диагностика по пользовательскому crop, authoritative UI и residual-карте показала, что скругление карточки корректно, а полоса была low-frequency эхом прежней generated-границы в однотонном промежутке перед `For you`. Создана source-space маска только для точного peach background (`±3` уровня) в диапазоне `y=1690…2040`, с небольшим dilation/feather у контрастного края; UI, текст и геометрия не перегенерировались. Проверки: residual mean в gap `6.967 → 0.409`, визуальный 2× before/after, полный master и финальный WebP-crop без полосы; вне телефона изменённых пикселей `0`. Обновлены canonical 1×/2× master и runtime `cover.webp` (`326938` bytes, PSNR `42.5162 dB`, SHA-256 `368dfa9005b6f27bd56452181eb7a81ffc95e28b6071741e8f777fc747abae10`). Предыдущая версия сохранена как `cover-clean-before-card-gap-fix.webp` и соседние PNG с суффиксом `before-card-gap-fix`.

- 2026-08-05: После пользовательской проверки удалены остаточные затемнения в левом и правом верхних углах дисплея GoomY cover. Причина совпала с Island-артефактом: low-frequency glass/color residual оставался на однотонном header за пределами локальной ellipse. Вместо новой точечной маски создана source-space маска всего header (`y=0…470` в authoritative UI), спроецированная той же homography и мягко сведённая перед hero-card (`sigma=30px @2x`); Dynamic Island по-прежнему возвращается отдельно только физическим pill. Независимые проверки: визуальное сравнение обоих 2× углов, численное residual mean слева `9.761 → 1.796` и справа `10.067 → 1.980`, полный master/runtime без видимого перехода; Island core и область вне дисплея сохранили `max delta = 0`. Обновлены canonical 1×/2× master и runtime `cover.webp` (`327088` bytes, PSNR `42.5119 dB`, SHA-256 `3218bbdf38e88beee7f9df60224196d43ef84097fe39d838daf4403382c0383e`). Предыдущая версия сохранена как `cover-clean-before-header-corners-fix.webp` и соседние PNG с суффиксом `before-header-corners-fix`.

- 2026-08-05: Повторно исправлено пятно вокруг Dynamic Island в финальном GoomY cover после подтверждения пользователя, что mask-only версия визуально не изменилась. Диагностика показала реальный источник: широкий low-frequency glass/color residual (`low_base - low_ui`, `sigma=48px @2x`) продолжал возвращать тот же тёпло-серый градиент поверх clean UI; узкая маска pill сама по себе этого не устраняла. Residual подавлен в локальной эллиптической feathered-зоне вокруг Island (`sigma=34px @2x`), после чего из исходного мокапа возвращён только физический чёрный component. Проверки: residual mean в зоне `23.588 → 2.603`, Island core `max delta = 0`, вне дисплея `max delta = 0`; крупный 2× before/after и полный runtime WebP визуально проверены — ореол удалён, переходов и изменений текста нет. Обновлены canonical `final-natural-light-crop-ui-clean.png` (`1323×1189`), 2× master (`2646×2378`) и `public/media/cases/goomy/card/cover.webp` (`327180` bytes, PSNR `42.5069 dB`, SHA-256 `77385ca5de28f80bf017103a1ba9ce2ba341692a4a31aa67d71c978feb83567d`). Предыдущие master/runtime сохранены с суффиксом `before-residual-fix`.

- 2026-08-05: Убрано тёплое пятно/ореол вокруг Dynamic Island в clean GoomY cover. Причина: прежняя `17×17` dilation исключала из UI-композита широкую область вокруг Island и сохраняла там фрагмент исходного generated-фона. Новый порядок: clean UI непрерывно заполняет фон под Island, затем из исходного мокапа возвращается только физический чёрный component через `7×7` dilation + `sigma=1` feather. Крупный crop и итоговый WebP визуально проверены; rounded screen, статусные иконки и текст не затронуты, Island core `max delta = 0`, вне дисплея `max delta = 0`. Обновлены canonical `final-natural-light-crop-ui-clean.png`, `final-natural-light-crop-ui-clean-2x.png` и runtime `cover.webp` (`327380` bytes, PSNR `42.5034 dB`, SHA-256 `d26bc344dcb399315095b7b13f8ecd056db897bcaf9dfb4a475a48e96a69383c`). Предыдущий clean runtime сохранён как `cover-clean-before-island-fix.webp`.

- 2026-08-05: Переделан неудачный GoomY high-res composite с motion-blur/тенями текста. Причина: прежний strength `0.68` смешивал authoritative sharp UI с generated blurred UI, поэтому субпиксельное несовпадение сохраняло двойные контуры. Новый clean-пайплайн полностью заменяет UI внутри source-space rounded-screen mask (`radius=150`) по той же SIFT/RANSAC-homography (`554` inliers, median `0.839px`, p95 `2.088px`), а из Natural-мокапа возвращает только нормализованный very-low-frequency glass/color residual (`sigma=48px @2x`, clamp `±32`). Dynamic Island core и область вне дисплея: `max delta = 0`; визуально проверены текст, скругления, bezel и WebP. Сохранены `final-natural-light-crop-ui-clean.png` (`1323×1189`) и `final-natural-light-crop-ui-clean-2x.png` (`2646×2378`); runtime `cover.webp` заменён (`327596` bytes, PSNR `42.5038 dB`, SHA-256 `5113f555e27af0fc2168f8b006e1edcd8890af69badd6f65e1083e7d3f77c7bd`). Ghosted runtime сохранён как `cover-ui-hires-ghosted.webp`.

- 2026-08-05: Исправлена мягкость UI в утверждённом Natural-light GoomY cover детерминированной high-resolution репроекцией authoritative `/Users/vladyslavhorovyy/Desktop/goomy case/cover/Explore for mock.png`. SIFT/RANSAC-homography: `684` ratio-test matches, `554` inliers, median error `0.839px`, p95 `2.088px`; high-frequency UI detail перенесён с strength `0.68` поверх сохранённого low-frequency света/стекла. Созданы `final-natural-light-crop-ui-hires.png` (`1323×1189`) и `final-natural-light-crop-ui-hires-2x.png` (`2646×2378`). QA: вне дисплея `max delta = 0`, Dynamic Island core `max delta = 0`, screen Laplacian variance `122.44 → 519.09`, runtime WebP визуально проверен, PSNR к master `43.1393 dB`. `public/media/cases/goomy/card/cover.webp` заменён (`320790` bytes, SHA-256 `aa5b0a99d7e7643f8b363e77ad1e92dd2d0d0a8103f224675093cc7a14ae98c9`); soft runtime сохранён как `cover-soft-before-ui-hires.webp`.

- 2026-08-05: Диагностирована мягкость UI в финализированном Natural-light cover: текущий пайплайн использовал только кроп, Lanczos-resize `1251×1122 → 1323×1189` и WebP q95, без прежней high-resolution репроекции интерфейса. Для сравнения, резкий `final-ui-hires-2x-clean.png` был построен из authoritative UI `Explore for mock.png` через RANSAC-homography (`512` inliers, median error `0.415px`, p95 `1.341px`) с переносом только high-frequency детализации, сохранением glass residual и защитой Dynamic Island. Runtime на этом диагностическом шаге не изменялся.

- 2026-08-05: Финализирован утверждённый пользователем Natural-light кроп GoomY cover из `/Users/vladyslavhorovyy/Downloads/image 110.png` (`1251×1122`). Для точного контракта удалены только 3 крайних пикселя по ширине, выполнен Lanczos-ресайз в `1323×1189`, сохранён master `/Users/vladyslavhorovyy/Desktop/goomy case/cover/v4/final-natural-light-crop-1323x1189.png` и заменён runtime `public/media/cases/goomy/card/cover.webp` (`WebP q95`, `316338` bytes, SHA-256 `95d3a404502a6c673b9fce524307d10a4dc42f22c007e900aed8c960127b4330`). Предыдущий runtime сохранён как `cover-before-final-natural-crop.webp`. Проверки: размеры/формат/alpha — корректны; оригинальный WebP визуально проверен; PSNR к master `43.6393 dB`; полные `npm run build` и `npx astro build` были остановлены, потому что оба независимо зависли без CPU/вывода соответственно в prebuild `generate-agent-readable-content.mjs` и Astro runtime — это отмечено как внешняя проблема окружения, не связанная со статическим cover-файлом.

- 2026-08-05: По утверждённому Natural/Studio spec созданы два локальных ImageGen preview на композиции с реалистичной проекцией телефона: `goomy-muted-lime-realistic-projection-natural-preview-v10.png` (`1324×1188`, направленный дневной свет и мягкие тени листвы) и `goomy-muted-lime-realistic-projection-studio-preview-v10.png` (`1324×1188`, мягкий студийный key/fill без листвы). В обоих сохранены muted-lime матовый материал, минимальная wall/floor сцена и GoomY UI; production-нормализация, high-res composite и замена сайта не выполнялись. SHA-256 runtime `cover.webp` остался `cb313c3c603f300f5fc269d80a1687f02d43fc43beb604db732e47c119967058`.

- 2026-08-05: После возврата к композиции с более реалистичной проекцией телефона утверждён дизайн двух GoomY preview-вариантов — Natural и Studio. Общими инвариантами зафиксированы композиция, проекция, muted-lime палитра, гладкий матовый материал и неизменный UI; различается только модель света. Spec: `docs/superpowers/specs/2026-08-05-goomy-cover-natural-studio-light-design.md`. Генерация и runtime-замена на этом шаге не выполнялись.

- 2026-08-05: Создан и сохранён отдельный ImageGen preview `goomy-muted-lime-centered-flush-generated-v9.png` (`1324×1188`): визуальная масса телефона перенесена в центр кадра, задняя плоскость выровнена параллельно вертикальной грани с широким контактом; диагональное ребро, muted-lime палитра и живой солнечный свет сохранены. Точный production-размер будет нормализован только после утверждения генерации; финализация и замена runtime-обложки не выполнялись, SHA-256 `cover.webp` остался `cb313c3c603f300f5fc269d80a1687f02d43fc43beb604db732e47c119967058`.

- 2026-08-05: Создан preview `goomy-muted-lime-camera-right-closer-generated-v8.png` (`1323×1189`): относительно v7 камера смещена вправо и приближена, телефон увеличен до hero-scale; выбранные диагональное ребро и внутренний угол ступени сохранены. Финализация не выполнялась, runtime checksum не изменился.

- 2026-08-05: Создан preview `goomy-muted-lime-locked-corner-closer-generated-v7.png` (`1323×1189`): сохранены выбранные диагональное верхнее ребро, острый внутренний угол и перспектива ступени; камера приближена, телефон увеличен и смещён к оптическому центру. Финализация не выполнялась, runtime checksum не изменился.

- 2026-08-05: Создан preview-кандидат `goomy-muted-lime-closer-centered-generated-v6.png` (`1323×1189`) на основе выбранного landscape-ракурса: камера приближена, телефон увеличен и сдвинут к оптическому центру, контакт со ступенью выровнен по ребру; muted-lime палитра и живой солнечный свет сохранены. High-res финализация не выполнялась; runtime checksum не изменился.

- 2026-08-05: Для muted-lime GoomY set созданы два дополнительных ImageGen preview без финализации: `goomy-muted-lime-angle-top-generated-v4.png` (`1323×1189`, слегка верхний диагональный ракурс) и `goomy-muted-lime-angle-low-landscape-generated-v5.png` (`1324×1188`, более низкий трёхчетвертной ракурс). Отклонённый portrait draft не предлагается как кандидат. High-res composite/runtime не менялись; checksum `cover.webp` остался `cb313c3c603f300f5fc269d80a1687f02d43fc43beb604db732e47c119967058`.

- 2026-08-05: По пользовательской корректировке остановлена преждевременная финализация GoomY cover. Создан и сохранён только ImageGen preview `goomy-muted-lime-live-light-generated-v2.png` (`1323×1189`): одна крупная архитектурная ступень, приглушённый lime, направленный солнечный свет и мягкие тени листвы по новому референсу. High-res композит не выполнялся; runtime checksum остался `cb313c3c603f300f5fc269d80a1687f02d43fc43beb604db732e47c119967058`.

- 2026-08-05: Утверждённая арт-дирекция двух GoomY cover-итераций оформлена в `docs/superpowers/specs/2026-08-05-goomy-geometric-cover-iterations-design.md`, а исполнимый ImageGen/compositing/QA-план — в `docs/superpowers/plans/2026-08-05-goomy-geometric-cover-iterations.md`. План покрывает Geometric studio и Full abstract, точный возврат 2× телефона, численные alignment/pixel-diff gates и запрет изменения runtime-обложки до отдельного подтверждения.

- 2026-08-05: Собран локальный GoomY cover `final-ui-original-colors-brighter-hires-v3.png` (`2646×2378`): светлое окружение и контактная тень взяты из ImageGen-регенерации `v2`, а весь телефон/UI — из `final-ui-hires-2x-clean.png`. Геометрия совмещена по 1327 RANSAC-inliers (median residual `0.751px`, p95 `1.639px`); внутри core-маски телефона `max delta = 0`, изменённых каналов `0`. Runtime-обложка сайта не заменялась.

- 2026-08-05: Для GoomY cover создан локальный ImageGen-вариант `final-ui-original-colors-brighter-regenerated-v2.png`: сохранены исходные оливково-зелёный и карамельно-песочный цвета окружения, а сцена перегенерирована с более светлым рассеянным освещением и мягкой контактной тенью. Телефон/UI зафиксированы в промпте как неизменяемые; runtime `public/media/cases/goomy/card/cover.webp` не заменялся, его SHA-256 остался `cb313c3c603f300f5fc269d80a1687f02d43fc43beb604db732e47c119967058`. Генератор выдал preview `1322×1190`; точный production-размер будет нормализован только после выбора финала.
...
- 2026-06-06: Описан PRD для agent-readable слоя портфолио без Agent Skill.
  Причина: пользователь решил сделать сайт более нативным для AI-агентов рекрутеров, но исключить отдельный installable Agent Skill из scope.
  Файлы: `tasks/agent-readable-site-prd.md`, `tasks/logs.md`.
  Что сделано: зафиксированы цели, не-цели, аудит текущего состояния, информационная архитектура (`/llms.txt`, `/llms-full.txt`, markdown-страницы, `/agent-profile.md`), требования к metadata/JSON-LD, генерации, acceptance criteria, проверкам и этапам внедрения.
  Проверки: документ создан и перечитан; `git check-ignore -v tasks/agent-readable-site-prd.md` подтвердил, что новый файл подпадает под существующее ignore-правило `/tasks/`; код сайта не изменялся.
- 2026-06-06: PRD agent-readable слоя дополнен карьерным позиционированием из `Vacancies`.
  Причина: пользователь попросил проверить полноту PRD и использовать папку `Vacancies` как источник сильных сторон, target roles и positioning.
  Файлы: `tasks/agent-readable-site-prd.md`, `tasks/logs.md`.
  Что сделано: добавлены источники данных и приоритеты, hiring narrative, role-fit taxonomy, proof matrix, public/private boundary, требования к `agent-profile.md`/`llms-full.txt`, проверки на утечку внутренних job-search данных и новые open questions по claims `5+ years`, `700K MAU`, Fozzy Group, MVP и canonical name.
  Проверки: перечитаны `Vacancies` docs (`candidate-profile-assets`, `vacancy-criteria`, `outreach-system`, cover-letter guides, job-search gaps); код сайта не изменялся.
- 2026-06-06: Зафиксированы решения по agent-readable PRD и добавлена аудитория startup CEO/founder.
  Причина: пользователь подтвердил публичность claims `5+ years`, `700K MAU`, Fozzy Group и MVP; выбрал canonical official name `Vladyslav Horovyy`; уточнил, что one-line summary не должен включать `Senior`; дополнительно попросил учесть интерес CEO стартапов.
  Файлы: `tasks/agent-readable-site-prd.md`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: обновлены narrative, audience, founder/CEO-ready summary, proof matrix, JSON-LD name/alternateName, acceptance criteria, manual QA и open questions; в `tasks/lessons.md` добавлено краткое устойчивое правило.
  Проверки: документ перечитан выборочно через `rg`/`sed`; код сайта не изменялся.
- 2026-06-06: Уточнены primary startup/CEO сценарии для agent-readable PRD.
  Причина: пользователь выбрал фокус CEO-нарратива ближе к `mobile B2C app`, `founder-style 0-to-1`, `craft-heavy product polish`.
  Файлы: `tasks/agent-readable-site-prd.md`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: обновлены founder/CEO audience, summary, primary/secondary startup-сценарии, proof matrix и manual QA; `tasks/lessons.md` синхронизирован с устойчивым правилом.
  Проверки: точечная правка документации; код сайта не изменялся.
- 2026-06-06: CEO/founder narrative уточнён с `founder-style 0-to-1` на `0-to-1 product design`.
  Причина: пользователь уточнил, что позиционирование должно оставаться дизайнерским; `founder-style` может ошибочно звучать как claim на founder/operator роль.
  Файлы: `tasks/agent-readable-site-prd.md`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: заменены формулировки в audience, founder/CEO-ready summary, primary scenarios, proof matrix и lessons; акцент сохранён на 0-to-1 product design / early product shaping без founder-style overclaim.
  Проверки: точечная документационная правка; код сайта не изменялся.
...
- 2026-03-27: Реализован последовательный переход с `CaseCard` по контракту `case-switcher` (`dissolve -> navigate -> gated intro`) на маршрутах `/` и `/cases`.
  Причина: требовалось убрать «soft» переход при клике на кейс-карточку и синхронизировать поведение с уже принятым контрактом `case-switcher`.
  Файлы: `src/components/CaseCard.astro`, `tests/smoke/case-details.spec.ts`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: (1) в runtime `CaseCard` добавлен route-intercept клика (capture phase) с `navigate()` через Astro transitions; (2) добавлен pre-fade сценарий с `data-case-switcher-leaving`, ожиданием `transitionend/cancel` по `opacity` и fallback-таймерами; (3) добавлен marker `__case-switcher-intro-sync` перед навигацией и guard на `astro:before-swap` для входящей detail-страницы (`data-case-switcher-entering='true'`), плюс cleanup lock/timers на `pagehide`; (4) сохранён reduced-motion контракт как у switcher (без pre-fade, сразу навигация); (5) добавлен smoke-тест, проверяющий переход по `CaseCard` из `/` и `/cases`: transition-window, marker/gate, отсутствие раннего старта intro, сброс `scrollY` и очистку marker.
  Проверки: (1) `npm run build` — успешно; (2) `npx playwright test tests/smoke/case-details.spec.ts --grep "case card click from home and cases uses sequential fade->navigate transition|/fora case switcher next waits for fade-end and starts intro after route transition"` — успешно (`2 passed`).
- 2026-06-06: Усилен social preview для X/Twitter и добавлен cache-bust URL cover-изображения.
  Причина: X показывал link card без cover; live-диагностика подтвердила наличие старого `og:image/twitter:image`, поэтому наиболее вероятная причина — кеш карточки X.
  Файлы: `src/layouts/BaseLayout.astro`, `scripts/verify-social-meta.mjs`, `package.json`, `tasks/logs.md`.
  Что сделано: (1) `socialCoverUrl` переведён на versioned URL `/media/site/site-cover.png?v=2026-06-06`; (2) добавлены явные Twitter Card теги `twitter:title`, `twitter:description`, `twitter:image:alt`; (3) добавлен `verify:social-meta` для проверки собранного `dist/index.html` и `verify:social-meta:source` для source-level guard.
  Проверки: (1) до исправления `node_modules` импорт `@tailwindcss/vite` занимал ~2 минуты, а `astro.config.mjs` не завершался; контрольный чистый install в `/tmp` импортировал config за ~1s; (2) `npm ci --ignore-scripts` в рабочем репозитории восстановил `node_modules` (`5.6M -> 172M`), после чего `@tailwindcss/vite` импортируется за ~1s; (3) `npm run build && npm run verify:social-meta` — успешно (`6 page(s) built`, `Social metadata looks good`); (4) `curl -I https://vladhorovyy.com/media/site/site-cover.png?v=2026-06-06` — `200 OK`, `content-type: image/png`.
- 2026-06-06: Очищены локальные Playwright artifacts из git-состояния.
  Причина: `test-results/` и пустой файл `down` засоряли `git status`; один Playwright state-файл (`test-results/.last-run.json`) был tracked и мешал полностью игнорировать артефакты.
  Файлы: `.gitignore`, `test-results/.last-run.json`, `tasks/logs.md`.
  Что сделано: (1) добавлен ignore-паттерн `test-results/`; (2) удалены локальные `test-results/` и `down`; (3) tracked `test-results/.last-run.json` удалён из репозитория как runtime/test artifact.
  Проверки: (1) `git status --short --untracked-files=all` — в статусе остались только осознанные файлы фикса и удаление tracked Playwright artifact; (2) `npm run build && npm run verify:social-meta` — успешно (`6 page(s) built`, `Social metadata looks good`).
- 2026-06-06: Реализован agent-readable слой портфолио и page-specific AI/SEO metadata.
  Причина: сайт должен быстро и однозначно читаться агентами рекрутеров, hiring managers и startup CEO/founder-аудитории, без утечки приватного job-search контекста.
  Файлы: `public/{llms.txt,llms-full.txt,agent-profile.md,fora.md,kissa.md,gallery.md}`, `scripts/generate-agent-readable-content.mjs`, `scripts/verify-agent-readable.mjs`, `src/data/{agent-profile.ts,agent-proof.ts,structured-data.ts}`, `src/layouts/BaseLayout.astro`, `src/pages/{index.astro,cases.astro,gallery.astro,[slug].astro}`, `src/data/gallery.ts`, `scripts/verify-social-meta.mjs`, `package.json`, `tests/smoke/{home-hero-cta-animation.spec.ts,gallery.spec.ts,case-details.spec.ts,theme-tokens.spec.ts}`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: (1) добавлен генератор agent-readable markdown/txt слоя (`llms.txt`, full dossier, canonical profile, case dossiers, gallery caveat) из whitelisted public data; (2) добавлен privacy/content verifier для source и dist с проверкой public facts, absolute links, founder/CEO summary, кейсовых outcomes и запретных internal markers; (3) `prebuild` теперь генерирует и проверяет agent-readable слой; (4) `BaseLayout` получил page-specific `title/description/structuredData`, canonical name обновлён на `Vladyslav Horovyy`, а страницы подключили ProfilePage/Person и CreativeWork JSON-LD; (5) smoke title expectations синхронизированы с новыми route-specific titles; (6) в gallery alt заменён двусмысленный `finance tracker` на `finance dashboard`, чтобы crawled HTML не цеплял internal marker.
  Проверки: (1) `npm run verify:agent-readable:source && npm run verify:social-meta:source` — успешно; (2) `npm run build` — успешно (`6 page(s) built`); (3) `npm run verify:agent-readable && npm run verify:social-meta` — успешно; (4) privacy grep по `public` и `dist` на forbidden markers — без совпадений; (5) targeted smoke `home-hero-cta-animation + gallery + case-details + theme-tokens` — успешно (`54 passed`, `4 skipped`); (6) полный `npm run test:smoke` — `85 passed`, `4 skipped`, `2 failed` в parallel timing-сценариях; оба упавших теста прошли обязательный изолированный ретрай (`header-appear`, `case card click...` по `1 passed` каждый).
- 2026-06-06: Проведён PRD-аудит agent-readable реализации и усилены найденные слабые места.
  Причина: пользователь попросил детально проверить полноту/качество реализации PRD и внести правки при необходимости.
  Файлы: `scripts/generate-agent-readable-content.mjs`, `scripts/verify-agent-readable.mjs`, `src/data/structured-data.ts`, `public/llms-full.txt`, `tasks/logs.md`.
  Что сделано: (1) через subagent-а проведён независимый read-only audit generated/public/dist artifacts; (2) найден и закрыт quality gap: `llms-full.txt` был короче PRD-ориентира, расширен с ~993 до `1899` слов и теперь включает fast-read, recruiter/CEO answer patterns, case proof details, working style, tools context и role-fit guidance; (3) JSON-LD для `ProfilePage`/`CreativeWork` расширен PRD-полями (`headline`, `abstract`, `author`, `about`, `mainEntityOfPage`); (4) `verify-agent-readable` усилен проверкой word count, line count `llms.txt`, JSON-LD parse/required fields, отсутствия markdown runtime/CSS шума и forbidden markers в dist HTML; (5) live `curl -I https://vladhorovyy.com/llms.txt` подтвердил `404` на текущем проде — это deployment gap до публикации изменений, не локальная ошибка сборки.
  Проверки: (1) `npm run generate:agent-readable && npm run verify:agent-readable:source` — успешно, `public/llms-full.txt = 1899` слов; (2) `npm run build && npm run verify:agent-readable && npm run verify:social-meta && npm run verify:social-meta:source` — успешно; (3) targeted smoke `home-hero-cta-animation + gallery + case-details` — успешно (`44 passed`, `4 skipped`); (4) `git diff --check` — без замечаний; (5) privacy grep по `public` и `dist` — без совпадений.
- 2026-06-06: Смягчён agent-readable guardrail про AI/3D/motion.
  Причина: пользователь заметил, что строка `Do not present AI, 3D, or motion as...` может слишком зажимать агента и недопродавать craft/AI как дифференциатор.
  Файлы: `src/data/agent-profile.ts`, `public/{llms.txt,llms-full.txt,agent-profile.md}`, `tasks/logs.md`.
  Что сделано: формулировка изменена с запрета на "main argument" на более точный guardrail: не фреймить AI/3D/motion как isolated gimmicks, а подавать как product-craft differentiators, связанные с clarity, prototyping, visual systems, shipping и interface quality.
  Проверки: `npm run generate:agent-readable && npm run verify:agent-readable:source` — успешно.
- 2026-06-06: Пересмотрены все публичные agent-readable запреты/guardrails.
  Причина: нужно было убедиться, что ограничения защищают от фактических ошибок и приватных утечек, но не занижают positioning и craft/AI/3D/motion narrative.
  Файлы: `src/data/agent-profile.ts`, `src/data/agent-proof.ts`, `scripts/generate-agent-readable-content.mjs`, `scripts/verify-agent-readable.mjs`, `public/{llms.txt,llms-full.txt,agent-profile.md,fora.md,gallery.md}`, `tasks/logs.md`.
  Что сделано: (1) блоки `Do Not Overclaim`/`What Not To Overclaim` переименованы в `Accuracy Guardrails`; (2) жёсткие запреты переписаны как точный framing: role guardrail оставлен строгим только для frontend/software engineer positioning, метрики описываются как project/team outcomes, gallery — как craft/concept/product-polish signal, приватность — без перечисления pay/recruiter/internal scoring конкретики, AI/3D/motion — как product-craft differentiators; (3) case/gallery caveats и tools context переписаны менее defensive; (4) verifier синхронизирован с новой role guardrail-строкой.
  Проверки: (1) `npm run build && npm run verify:agent-readable && npm run verify:social-meta` — успешно; (2) grep по `public/dist` на старые жёсткие формулировки и приватные конкретики — без совпадений; (3) `git diff --check` — без замечаний.
- 2026-06-06: Убраны публичные упоминания privacy/job-search boundary из внешних agent-readable файлов.
  Причина: пользователь уточнил, что внутренний job-search контекст не должен лежать во внешних доках портфолио даже в виде guardrail-формулировок.
  Файлы: `src/data/agent-profile.ts`, `scripts/generate-agent-readable-content.mjs`, `public/{llms.txt,llms-full.txt,agent-profile.md}`, `tasks/logs.md`.
  Что сделано: (1) из публичного массива `doNotOverclaim` удалён пункт про private career-search operations/internal criteria; (2) из генератора удалены секции `Public/Private Boundary` для `agent-profile.md` и `llms-full.txt`; (3) privacy enforcement оставлен только во внутренних местах: PRD/lessons/verifier.
  Проверки: (1) `npm run generate:agent-readable && npm run verify:agent-readable:source` — успешно; (2) `npm run build && npm run verify:agent-readable && npm run verify:social-meta` — успешно; (3) grep по `public` и `dist` на `private/career-search/job-search/internal decision/Public/Private/pay preferences/internal scoring/recruiter notes/salary/compensation` — без совпадений.
- 2026-06-06: Проведён финальный review agent-readable docs и production bundle перед завершением.
  Причина: пользователь попросил последний детальный ревью-проход с subagent-аудитом и финальными правками, если они нужны.
  Файлы: `src/data/agent-profile.ts`, `src/data/agent-proof.ts`, `src/data/case-process/fora.ts`, `src/layouts/BaseLayout.astro`, `src/components/dev/AgentationMount.astro`, `src/styles/base.css`, `scripts/generate-agent-readable-content.mjs`, `scripts/verify-agent-readable.mjs`, `tasks/agent-readable-site-prd.md`, `public/{llms.txt,llms-full.txt,agent-profile.md,fora.md,kissa.md,gallery.md}`, `tasks/logs.md`.
  Что сделано: (1) по technical subagent-аудиту усилен verifier: сравнение `public`/`dist` для generated docs, dist-scan на dev/Agentation markers и расширенный denylist приватных job-search фрагментов; (2) публичная copy очищена от внутренних meta-формулировок и defensive wording (`approved as...`, `Weaker Fits`, старые `Caveat` headings, `not about decoration`); (3) PRD синхронизирован с решением не публиковать public/private boundary во внешних agent-readable файлах; (4) метрика Fora в process-data выровнена с публичным case outcome (`UI bugs -40%`); (5) Agentation dev toolbar вынесен в dev-only dynamic mount, а production CSS очищен от `agentation-dev-root`.
  Проверки: (1) `npm run generate:agent-readable && npm run verify:agent-readable:source && npm run build && npm run verify:agent-readable && npm run verify:social-meta` — успешно; (2) targeted smoke `case-details + gallery + home-hero-cta-animation` — успешно (`44 passed`, `4 skipped`); (3) grep по `dist` на `Agentation|agentation-dev-root|localhost:4747|PUBLIC_AGENTATION` — без совпадений; (4) grep по `public/dist` на приватные/internal и старые defensive формулировки — без совпадений; (5) `git diff --check` — без замечаний.
- 2026-06-11: Усилен discovery и снижен шум agent-readable слоя после live-аудита сайта внешним агентом.
  Причина: внешний агент, который читает только HTML, не видел прямой подсказки на `/llms.txt`, а `agent-profile.md` и `llms-full.txt` частично дублировали proof matrix.
  Файлы: `src/layouts/BaseLayout.astro`, `public/{robots.txt,sitemap.xml,llms.txt,llms-full.txt,agent-profile.md,kissa.md}`, `scripts/generate-agent-readable-content.mjs`, `scripts/verify-agent-readable.mjs`, `src/data/{agent-proof.ts,gallery.ts}`, `src/data/case-details/kissa.ts`, `tasks/lessons.md`, `tasks/logs.md`.
  Что сделано: (1) добавлены невидимые `<link rel="alternate">` на `/llms.txt` и `/agent-profile.md` во все страницы через `BaseLayout`; (2) agent-readable docs добавлены в `sitemap.xml`, а `/llms.txt` указан в `robots.txt`; (3) `agent-profile.md` сокращён за счёт замены полной proof matrix на компактный `Proof Routing` и сжатия role/core-strength списков, подробная матрица оставлена в `llms-full.txt`; (4) `llms-full.txt` получил ссылки на case docs в `Case Summaries`; (5) исправлены copy/typo: `speeded up flow times` -> `reduced flow time`, `tap error rate by 80%` -> `reduced tap error rate by 80%`, `socioligy` -> `sociology`; (6) verifier усилен проверками discovery в head/sitemap/robots и запретом старых typo/copy фрагментов.
  Проверки: (1) `npm run generate:agent-readable && npm run verify:agent-readable:source` — успешно; (2) `npm run build && npm run verify:agent-readable && npm run verify:social-meta` — успешно; (3) targeted smoke `case-details + gallery` — успешно (`41 passed`, `4 skipped`); (4) `wc -w` подтвердил сокращение `agent-profile.md` до `800` слов при сохранении `llms-full.txt` `1863` слов; (5) head/sitemap/robots sanity — discovery ссылки присутствуют; (6) дословные длинные повторы между `agent-profile.md` и `llms-full.txt` снижены с `25` до `12`, оставлены только canonical summary/facts/guardrails.
- 2026-06-11: Финальная source-data корректировка Kissa после live/deploy double-check.
  Причина: live agent-readable слой был опубликован и проходил discovery-проверки, но в human case source оставалась менее аккуратная строка `reduce flow time by 50%`.
...
## 2026-08-05 — Альтернативный close-up crop GoomY cover

- Пользовательский кроп `/Users/vladyslavhorovyy/Downloads/image 626.png` подготовлен без регенерации в формате серии `1323×1189`: master PNG `final-natural-light-closeup-crop.png` и WebP `final-natural-light-closeup-crop.webp` сохранены в `/Users/vladyslavhorovyy/Desktop/goomy case/cover/v4/`.
- Использован Lanczos-downscale с центральным extent; композиция, UI и цвета не изменялись. Runtime-ассет сайта не заменялся.
- Проверки: оба файла имеют размер `1323×1189`; PNG визуально проверен — артефактов кропа/ресайза не обнаружено.

## 2026-08-05 — Публикация close-up GoomY cover
